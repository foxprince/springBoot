...
<c:import url="../include/head.jsp">
	<c:param name="pageTitle" value="渠道-业务代码配置"/>
</c:import>	
<body>
	<!-- topbar -->
		...
		<div class="row-fluid">
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						...
						<select name="channelId" class="form-control" onchange="window.location.href=(this.options[this.selectedIndex].value)">
							...
							<option value="/channelCode/?relateId=${item.key}" ${(not empty channel) && (item.key == channel.id) ? 'selected="selected"' : ''}>${item.value}</option>
							</c:forEach>
						</select>
			...
			<div class="row-fluid sortable ui-sortable" >
				<div class="box span12">		
					...
					<form:form method="POST" modelAttribute="channelCodeForm" action="/channelCode/addBatch">
					<c:forEach var="busi" items="${busiList}">
						<div class="box-header well" >
							<h2>
								<i class="icon-list"></i>  ${busi.name } 业务代码列表 
							</h2>
							<div class="box-icon">
						...
						<div class="box-content">
							<form:checkboxes  items="${busi.codeMap}" path="codeList" />
