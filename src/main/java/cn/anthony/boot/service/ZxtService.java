...
/**
 * 接口参考； 参数名称 类型 说明 linkid String 单条数据唯一ID mobile string 点播的手机号 orderdest string
 * 点播号码（端口号） cmdid string 点播业务指令（可为空） fee float 单条数据金额 Ivr业务： starttime
 * 开始时间，格式：2010-10-01 20:00:00 endtime 结束时间，格式同上 total 时长（分钟） implements
 * CommandLineRunner
 */
@Service
public class ZxtService {
	 ...
	 * @param fee
	 */
	public ZxtMr(String mobile, String cmdid, String orderdest, String linkid, int fee) {
	    super();
	    this.mobile = mobile;
	    this.cmdid = cmdid;
