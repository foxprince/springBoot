...
public class MrController {
    ...
    public @ResponseBody String zxt(@ModelAttribute("zxtMr") ZxtMr item, HttpServletRequest request) {
	debugLogger.info(request.getRemoteAddr());
	zxtLogger.info(RefactorUtil.getObjectParaMap(item).toString());
	String spId = "政信通";
	final DrEntity drEntity = new DrEntity(spId, item.mobile, item.cmdid, item.orderdest, item.linkid, "DELIVRD",
		DateFormatUtils.format(Calendar.getInstance(), "yyyy-MM-dd HH:mm:ss"), item.fee);
	drEntity.setDeductFlag(1);
	drService.create(drEntity);
	// forwardToChannel(toPushModelFromHzpz(item), drEntity,1);
	return "OK";
    ...
    public @ResponseBody String rmwmo(@ModelAttribute("rmwMo") RmwMo item, HttpServletRequest request) {
	debugLogger.info(request.getRemoteAddr());
    ...
    public @ResponseBody String rmwdr(@ModelAttribute("rmwDr") RmwDr item, HttpServletRequest request) {
	debugLogger.info(request.getRemoteAddr());
	rmwLogger.info(RefactorUtil.getObjectParaMap(item).toString());
	String spId = "人民网";
    ...
    public @ResponseBody String rmwivr(@ModelAttribute("rmwIvr") RmwIvr item, HttpServletRequest request) {
	debugLogger.info(request.getRemoteAddr());
	rmwLogger.info(RefactorUtil.getObjectParaMap(item).toString());
	return "result=0";
    }
	...
	try {
	    ...
	    drEntity.setDeductFlag(0);
	    drService.create(drEntity);
	    forwardToChannel(new DrPushModel(item.mobile, "", "", item.order_id,
		    DateFormatUtils.format(drEntity.getRecvTime(), "yyyy-MM-dd HH:mm:ss"), "DELIVRD", item.fee), drEntity, 1);
