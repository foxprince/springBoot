...
public abstract class GenericController<T> {
    ...
    public String edit(@RequestParam Long id, Model m, SessionStatus status) {
	m.addAttribute(getService().findById(id));
	return getFormView();
    }

    @RequestMapping(value = "/editInList", method = RequestMethod.GET)
    public String editInList(@RequestParam Long id, Model m, SessionStatus status) {
	m.addAttribute(getService().findById(id));
	return list(m);
    }
    ...
    public ModelAndView delete(@RequestParam Long id, final RedirectAttributes redirectAttributes, SessionStatus status) throws EntityNotFound {
