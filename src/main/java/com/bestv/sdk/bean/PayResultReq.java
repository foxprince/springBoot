...
/**
 * 
tradeCode	string	支付订单号（百视通平台订单id）
price	string	实际支付金额，金额单位：分
completeTime	string	支付完成时间(GMT 1970到支付完成毫秒数)
platform	string	平台名称: MIGU-DDO（咪咕DDO）
orderId	string	订单号（CP业务订单流水号）
passData	string	CP透传参数，UTF-8进行UrlEncode
platTradeCode	string	平台订单号(保留字段，暂不支持)
sign	string	数据签名，签名算法见：5.支付通知签名算法 
